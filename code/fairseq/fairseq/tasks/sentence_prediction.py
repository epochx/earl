...
class SentencePredictionConfig(FairseqDataclass):
    ...
    freeze: bool = field(
        default=False,
        metadata={"help": "if True, only train the classifier head"},
    )
...
class SentencePredictionTask(FairseqTask):
    ...
    def build_model(self, cfg):
        ...
        if self.cfg.freeze:
            logger.info(
                f"Making model parameters non-trainable except {self.cfg.classification_head_name}"
            )
            for name, parameter in model.named_parameters():
                if not self.cfg.classification_head_name in name:
                    parameter.requires_grad_(False)
                    logger.info(f"Parameter {name} made non-trainable")
        return model
