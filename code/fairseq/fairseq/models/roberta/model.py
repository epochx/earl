...
class RobertaModel(FairseqEncoderModel):
        ...
        **kwargs,
        ...
        if classification_head_name is None:
            x, extra = self.encoder(
                src_tokens, features_only, return_all_hiddens, **kwargs
            )
            return x, extra
            ...
            if "masked_tokens" in kwargs or isinstance(
                classification_head_name, list
            ):
                output = {}

                if "masked_tokens" in kwargs:
                    masked_tokens = kwargs["masked_tokens"]
                    masked_logits = self.encoder.output_layer(
                        x, masked_tokens=masked_tokens
                    )
                    output["masked_lm"] = masked_logits

                if not isinstance(classification_head_name, list):
                    classification_head_name = [classification_head_name]
                for classification_head_name_i in classification_head_name:
                    x_i = self.classification_heads[classification_head_name_i](
                        x
                    )
                    output[classification_head_name_i] = x_i
            else:
                output = self.classification_heads[classification_head_name](x)
        ...
        for layer in self.encoder.sentence_encoder.layers:
            ...
            for i in range(layer.self_attn.num_heads):
                ...
                norm_loss_layer += scaling * (
                    torch.sum(
                        torch.abs(
                            layer.self_attn.q_proj.weight[start_idx:end_idx,]
                        )
                    )
                    + torch.sum(
                        torch.abs(
                            layer.self_attn.q_proj.bias[start_idx:end_idx]
                        )
                    )
                )
                norm_loss_layer += scaling * (
                    torch.sum(
                        torch.abs(
                            layer.self_attn.k_proj.weight[start_idx:end_idx,]
                        )
                    )
                    + torch.sum(
                        torch.abs(
                            layer.self_attn.k_proj.bias[start_idx:end_idx]
                        )
                    )
                )
                norm_loss_layer += scaling * (
                    torch.sum(
                        torch.abs(
                            layer.self_attn.v_proj.weight[start_idx:end_idx,]
                        )
                    )
                    + torch.sum(
                        torch.abs(
                            layer.self_attn.v_proj.bias[start_idx:end_idx]
                        )
                    )
                )
    ...
    def register_classification_head(
        self, name, num_classes=None, inner_dim=None, pooling=True, **kwargs
    ):
        ...
        if hasattr(self, "classification_heads"):
            ...
            for k, v in cur_state.items():
                if prefix + "classification_heads." + k not in state_dict:
                    logger.info(
                        "Overwriting " + prefix + "classification_heads." + k
                    )
...
class RobertaEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary):
        ...
        self.sentence_encoder = self.build_encoder(
    ...
    def extract_features(self, src_tokens, return_all_hiddens=False, **kwargs):
        ...
        inner_states = (
            encoder_out["encoder_states"] if return_all_hiddens else None
        )
...
def base_architecture(args):
    ...
    args.encoder_ffn_embed_dim = safe_getattr(
        args, "encoder_ffn_embed_dim", 3072
    )
    args.encoder_attention_heads = safe_getattr(
    ...
    args.pooler_activation_fn = safe_getattr(
        args, "pooler_activation_fn", "tanh"
    )
    args.untie_weights_roberta = safe_getattr(
        args, "untie_weights_roberta", False
    )
    ...
    args.encoder_layers_to_keep = safe_getattr(
        args, "encoder_layers_to_keep", None
    )

    # Quantization noise config
    args.quant_noise_pq = safe_getattr(args, "quant_noise_pq", 0)
    args.quant_noise_pq_block_size = safe_getattr(
